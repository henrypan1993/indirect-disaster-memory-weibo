...
$$
\mathrm{Entropy}_i = \frac{-\sum_{k=1}^{K} p_{ik}\log p_{ik}}{\log K} \in [0,1].
$$
...
- **D2 specification:** 18 rows = for each of the same 9 cells, `entropy_norm` and `indirect_clean` from the engagement model that adds entropy to the H1 specification.
...
| K | τ | D1: T2 → entropy | D2: entropy → engagement | H1 after entropy adjustment |
|---|----|------------------:|-------------------------:|----------------------------:|
...
**Table C1 notes.** The fifth column is the indirect/mixed coefficient from the D2 engagement specification (H1 plus entropy), not a D2 focal claim. Direction of D1 is negative in 8 of 9 cells; magnitude is τ-sensitive. Entropy never significantly predicts engagement; the H1 indirect/mixed coefficient remains stable.

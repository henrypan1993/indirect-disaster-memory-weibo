"""Fit confirmatory models: H1, H2-M1, D1-M1 (from model_data_final)."""
...
def run_all_specs(
    ...
    for spec in main_model_specs():
        ...
        rows, summary, _ = run_model_spec(df, spec, sid, meta_dir=OUT_MODELS)
        main_rows.extend(rows)
        texts[f"{spec.model_id}.txt"] = summary
        # Also write spec-tagged dump for clarity when multiple specs share model_id
        texts[f"{spec.model_id}_{sid}.txt"] = summary

    return main_rows, texts
...
def main() -> None:
    ...
    main_rows, texts = run_all_specs(df)
    ...
    print(f"main models -> {main_path} ({len(main_rows)} rows)")
